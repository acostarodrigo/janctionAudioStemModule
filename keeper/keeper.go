...
func NewKeeper(cdc codec.BinaryCodec, addressCodec address.Codec, storeService storetypes.KVStoreService, authority string, path string, bankKeeper bankkeeper.BaseKeeper) Keeper {
	...
	k := Keeper{
		...
		AudioStemTaskInfo: collections.NewItem(sb, audioStem.TaskInfoKey, "taskInfo", codec.CollValue[audioStem.AudioStemTaskInfo](cdc)),
		...
		Workers:           collections.NewMap(sb, audioStem.WorkerKey, "workers", collections.StringKey, codec.CollValue[audioStem.Worker](cdc)),
